...
public class IUserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> register(User user) {
        ...
        int  resoultCount = userMapper.insert(user);
        if(resoultCount == 0){
    ...
    public ServerResponse<String>checkValid(String str,String type){
        if(StringUtils.isNoneBlank(type)){
            ...
            if(Const.EMAIL.equals(type)){
                ...
                if(resoultCount > 0){
                    return ServerResponse.createByErrorMessage("用户输入邮箱已存在！");
                }
            }
        }else{
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user){
        //防止用户越权，要校验一下旧密码是否属于这个用户
        String md5password = MD5Util.MD5EncodeUtf8(passwordOld);
        int resultCount = userMapper.chackPassword(md5password,user.getId());
        if(resultCount  == 0){
